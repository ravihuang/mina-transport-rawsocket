...
public class RawIoConnector extends
    ...
    public RawPacket filter(PcapPacket packet) {
        RawPacket pkt = RawPacket.match_local_addr(packet, localAddr, broadcast,
                this.groupcast);
        
      //TODO 匹配其他包头字段
        if (DefaultRawSessionConfig.verbose)
            log.debug("{} match:{}", localAddr, packet);
        return pkt;
    }
    ...
    protected RawIoChannel newHandle(SocketAddress localAddress)
            throws Exception {
        RawIoChannel channel = new RawIoChannel(selector,tunnels);
